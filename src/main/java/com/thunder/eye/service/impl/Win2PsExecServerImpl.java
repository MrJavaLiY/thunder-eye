...
import com.thunder.eye.utils.PsExecCommandExecutor;
...
//@Service
@Slf4j
public class Win2PsExecServerImpl implements WinService {
    @Resource
    ...
    public ResponseEntity<List<JarDetailEntity>> dispatch(ServerConfig serverConfig) throws Exception {
        PsExecCommandExecutor psexec = new PsExecCommandExecutor(serverConfig.getIp(), serverConfig.getUsername(), serverConfig.getPassword());
        String jpsValue = psexec.exec("wmic process where \"name like '%java%'\" get ProcessId, CommandLine\n");
        ...
        for (String line : lines) {
            ...
            this.getJarMessage(psexec, entity);
            if (entity.getPort() == 0) {
                continue;
            }
    ...
    private void getJarMessage(PsExecCommandExecutor psexec, JarDetailEntity entity) throws Exception {
        String jvmmes = psexec.exec("jcmd " + entity.getPid() + " VM.command_line");
        ...
        this.getPost(psexec, entity);
    }

    private void getPost(PsExecCommandExecutor psexec, JarDetailEntity entity) throws Exception {
        if (entity.getJarName() != null && entity.getJarName().contains("org.jetbrains")) {
            return;
        }
        String netstat = "netstat -aon |findstr " + entity.getPid();
        String netstats = psexec.exec(netstat.toString());
        log.debug("===========netstat=======");
        log.debug(netstats);
        log.debug("=====================");
